...
function eventListeners() {
    ...
    $('#search-results').on('click', 'li', function() {
        if ($('#added-game-tags').find('span[appid=\'' + this.id + '\']').length > 0) {
            $('span[appid=\'' + this.id + '\']').remove();
            $(this).find('svg').toggle();
            $('#added-game-tags').trigger('contentChanged');
            return;
        }
        ...
        if ($(this).hasClass('search-results-item')) {
            var tag = '<span class=\'tag added-tag\' appid=\'' + this.id + '\'>' +
                ...
                '</span>';
            $('#added-game-tags').append(tag);
            $('#added-game-tags').trigger('contentChanged');
            $(this).find('svg').toggle();
        }
    });

    $('#added-game-tags').on('click', '.delete', function() {
    ...
    $(document).on('contentChanged', '#added-game-tags', function() {
        if ($('#added-game-tags').is(':empty')) {
            $('#add-game-submit').prop('disabled', true);
        } else {
